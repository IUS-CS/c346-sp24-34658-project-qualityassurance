...
struct NewHabitView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                        ...
                        Button(action: {
                            ...
                            Image(systemName: newHabit.icon)
                                ...
                                .sheet(isPresented: $isPresentingIconView, content: {
                                    SymbolsPicker(selection: $newHabit.icon, title: "Choose your symbol", autoDismiss: true)
                                }).padding()
                        }
                        Spacer()
                    }
                ...
                Section(header: Text("Date")) {
                    ...
                    // automatically set the due date to the end of the day
                    .onChange(of: newHabit.frequency) { _ in
                        if (newHabit.frequency == .daily) {
                            newHabit.dueDate = Calendar.current.startOfDay(for: Date())
                        } else if (newHabit.frequency == .weekly) {
                            newHabit.dueDate = Calendar.current.date(bySetting: .weekday, value: 7, of: Date())!
                        } else if (newHabit.frequency == .monthly) {
                            newHabit.dueDate = Calendar.current.date(byAdding: .month, value: 1, to: Date())!
                        }
                    }
                }
        ...
        //prefer light or dark depending on background color
